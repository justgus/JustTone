...
| --- | --- | --- |
| iPhone target | Configured | iOS 27.0, iPhone device family only; bundle ID `com.caposoft.JustTone`. |
| Watch companion | Configured | watchOS 27.0; bundle ID `com.caposoft.JustTone.watchkitapp`; companion ID points to the iPhone app and the Watch product is embedded. |
...
| App Group | Deferred | Select and provision the final JustTone-only group when EP-006 synchronization work requires it. No JustTune group may be reused. |
| Document type / exported UTI | Deferred | Define the `.justtone` type with import/export implementation in EP-004. |
| Microphone | Not applicable | Prohibited for JustTone by JT-SR-016; no usage description or entitlement is present. |
